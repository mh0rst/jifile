 ...
 * @since 1.5
 */
class JiFileModelSearch extends JModelLegacy
{
	/**
	 * Sezrch data array
	 *
	...
	function __construct()
	{
		...
		$this->setState('limit', $app->getUserStateFromRequest('com_jifile.limit', 'limit', $config->get('list_limit'), 'uint'));
		$this->setState('limitstart', JRequest::getUInt('limitstart', 0));
		
		// 20150112
		// managment searchphrase
		$searchphrase = $params->get('searchphrase');
		$searchphrase = (!empty($searchphrase)) ? $searchphrase : 'all'; 
		
		// Set the search parameters
		$keyword		= urldecode(JRequest::getString('searchword'));
		$match			= JRequest::getWord('searchphrase', $searchphrase);
		$ordering		= array(); //JRequest::getWord('ordering', 'newest');
