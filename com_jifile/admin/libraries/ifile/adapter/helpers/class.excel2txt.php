...
class Spreadsheet_Excel_Reader {
	...
	function dump_xls2txt($separetor = ' ')
	{ 
        $outs = array();
        $countSheet = count($this->sheets);
		
		for ($sheet = 0; $sheet < $countSheet; $sheet++) {
			for($row=1; $row<=$this->rowcount($sheet); $row++)
	        { 
	                $outs_inner = array();
	                for($col=1; $col<=$this->colcount($sheet); $col++)
	                { 
	                        ...
	                        if(empty($this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']))
	                        { 
	                                $val = $this->val($row, $col, $sheet);
	                ...
	                $outs[] = implode($separetor, $outs_inner); 
	        }
	        $out = implode("\r\n", $outs);
		}
		return($out);
	...
	function read($sFileName) {
		...
		if($res === false) {
			...
			if($this->_ole->error == 1) {
				...
				throw new Helper_Exception('The file is not readable');				
			}
